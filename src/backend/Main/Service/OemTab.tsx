...
import { getOemService, addOemService, deleteOemService, updateOemServiceOrderTable } from '@/server/service';
...
const OemTab: React.FC = () => {
  ...
  const [oemdata, setoemdata] = useState<DataType[]>([]);
  ...
  const fetchOemData = async () => {
    ...
    try {
      ...
      console.error('fetchOemData error', error);
  ...
  const handleAddOemData = async (values: any) => {
    try {
      await addOemService(values);
  ...
  const onDragEnd = async ({ active, over }: DragEndEvent) => {
    ...
    setoemdata((prev) => {
      ...
      // call API (assume updateOemServiceOrder expects array of ids)
      updateOemServiceOrderTable(orderIds)
        .then(() => {
  ...
  const columns: TableColumnsType<DataType> = [
    ...
    },
    {
      ...
      render: (_, record) => (
        <>
          {/* <Button type="text" icon={<EditOutlined />} onClick={() => navigate(`/oem/${record.id}`)} /> */}
  ...
  return (
    <>
      ...
      <DndContext sensors={sensors} modifiers={[restrictToVerticalAxis]} onDragEnd={onDragEnd}>
        <SortableContext items={oemdata.map((i) => String(i.id))} strategy={verticalListSortingStrategy}>
          <Table
            ...
            loading={loading}
            onRow={(record) => ({
              onClick: () => navigate(`/admin/oem/${record.id}`),
            })}
          />
        </SortableContext>
      </DndContext>
