...
const saveOrder = async (serviceId: number, order: number[]) => {
    try {
        await updateCollectionImageOrder(serviceId, order);
...
const ProductImgPage: React.FC = () => {
    ...
    const fetchService = async () => {
        ...
        try {
            setLoading(true);
            const res = await getCollectionImage(serviceId);
    ...
    const handleDelete = async (odmDetailId: number) => {
        try {
            await deleteCollectionImage(odmDetailId);
    ...
    const handleUpload = async () => {
        ...
        try {
            await uploadCollectionImageDetail(serviceId, fileObj as File);
    ...
    return (
        <Layout>
            ...
            <Content style={{ margin: "24px 16px", padding: 24, minHeight: 280 }}>
                <Title level={2}>Collection Image {id}</Title>
