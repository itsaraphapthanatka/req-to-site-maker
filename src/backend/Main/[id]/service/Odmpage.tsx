...
const saveOrder = async (serviceId: number, order: number[]) => {
    try {
        await updateOdmServiceOrder(serviceId, order);
...
const ServiceOdmPage: React.FC = () => {
    ...
    const fetchService = async () => {
        ...
        try {
            setLoading(true);
            const res = await getOdmServiceDetail(serviceId);
    ...
    const handleDelete = async (odmDetailId: number) => {
        try {
            await deleteOdmServiceDetail(odmDetailId);
    ...
    const handleUpload = async () => {
        ...
        try {
            await uploadOdmServiceImage(serviceId, fileObj as File);
    ...
    return (
        <Layout>
            ...
            <Content style={{ margin: "24px 16px", padding: 24, minHeight: 280 }}>
                <Title level={2}>ODM Service Detail {id}</Title>
