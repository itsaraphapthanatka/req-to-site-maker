...
// API call wrapper
const handleReorderInternationalStandard = async (order: number[]) => {
  await reorderInternationalStandard({ order });
};



// ========== MAIN COMPONENT ==========
const InternationalStandardTab: React.FC = () => {
  ...
  // ===== FETCH INTERNATIONAL STANDARD =====
  const fetchInternationalStandard = async () => {
    try {
      const resp = await getInternationalStandard();
  ...
  const handleUpload = async () => {
    ...
    try {
      await uploadInternationalStandardImage(formData);
  ...
  const onDragEnd = ({ active, over }: DragEndEvent) => {
    ...
    setData((prev) => {
      ...
      handleReorderInternationalStandard(newArr.map((i) => Number(i.key)));
  ...
  const handleDelete = async (id: number) => {
    try {
      await deleteInternationalStandard(id);
...
export { InternationalStandardTab };
