...
import { getProfessional, uploadProfessionalImage, reorderProfessional, deleteProfessional } from '@/server/professional';
...
// API call wrapper
const handleReorderProfessional = async (order: number[]) => {
  await reorderProfessional({ order });
};



// ========== MAIN COMPONENT ==========
const ProfessionalTeamTab: React.FC = () => {
  ...
  const fetchProfessional = async () => {
    try {
      const resp = await getProfessional();
  ...
  const handleUpload = async () => {
    ...
    try {
      await uploadProfessionalImage(formData);
  ...
  const onDragEnd = ({ active, over }: DragEndEvent) => {
    ...
    setData((prev) => {
      ...
      handleReorderProfessional(newArr.map((i) => Number(i.key)));
